    ...
    >
      ...
      >
        {numbersToNice.map((num) => {
          return <Tick key={`tick-${num}`} fontSize={fontSize}  >{num}</Tick>;
        })}
      </MotiView>
    </View>
  );
...
const Tick: React.FC<TextProps & {fontSize:number}> = ({ children, fontSize, ...rest }) => {
  return <Text {...rest} style={[
    ...
    }Ime&Cursor:PleaseSwitchTo1Ime&Cursor:PleaseSwitchTo1Ime&Cursor:PleaseSwitchTo1
  ]}>{children}</Text>;
};
