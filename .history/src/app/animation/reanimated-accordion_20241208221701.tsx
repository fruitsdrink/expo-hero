...
const TogglePage = () => {
  return (
    <Animated.ScrollView contentContainerStyle={{}}>
      ...
      >
        <DummyToggle title="Hello World" />
        <DummyToggle title="Hello Expo" />
        <Animated.View
          style={{
            padding: 12,
...
const DummyToggle = ({ type = "default", title }: DummyToggleProps) => {
  ...
  const { data, isFetching, isLoading, isRefetching, refetch } = useQuery({
    ...
    queryFn: async () => {
      const data = (await fetch(
        `https://dummyjson.com/quotes?limit=4&skip=${Math.floor(
          Math.random() * 20
        )}`
  ...
  return (
    <Toggle style={{ borderRadius: _spacing }}>
