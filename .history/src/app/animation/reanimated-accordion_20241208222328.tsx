  ...
  return (
    <Animated.ScrollView contentContainerStyle={{}}>
      ...
      >
        ...
        <DummyToggle title="Hello World" queryKey="1" />
        <DummyToggle title="Hello React Native" queryKey="2" />
        <DummyToggle title="Hello Expo" queryKey="3" />
        <Animated.View
          style={{
            padding: 12,
...
const DummyToggle = ({ title, queryKey }: DummyToggleProps) => {
  const [isActive, setIsActive] = useState(false);

  const { data, isFetching } = useQuery({
    queryKey: ["DummyData", queryKey, isActive],
    enabled: isActive,
    queryFn: async () => {
  ...
  useEffect(() => {
    if (!isActive) {
      queryClient.resetQueries({
        queryKey: ["DummyData", queryKey, true]
      });
    }
  }, [isActive]);
